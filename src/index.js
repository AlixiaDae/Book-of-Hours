...
import NoteStorage from "./modules/noteModules/noteStorage.js";
import Note from "./modules/noteModules/note.js";

//Local Storages

const storage = new Storage("hushhouse");
const noteStorage = new NoteStorage("notesStorage");
...
function createAspectTab(bookObject, tabName, box) {
  const memory = memoryList.find(
    (mem) => mem.memoryName === bookObject.memoryName
  );
  ...
  memoryName.textContent =
    bookObject.memoryName.charAt(0).toUpperCase() +
    bookObject.memoryName.slice(1);
...
function loadHush() {
  if (craftListEl.classList.contains("visible"))
...
craftTab.addEventListener("click", () => {
  if (aspectListEl.classList.contains("visible"))
...
function createNotesTab() {
  ...
  submitBtn.classList.add("submit-note");
  ...
  notesBox.append(form, notes);
  contentBox.appendChild(notesBox);
}

function createNotes(box) {
  const notesList = noteStorage.getNotes().notes.map((note) => note.getBody());

  for (let i = 0; i < notesList.length; i++) {
    const note = document.createElement("div");
    note.classList.add("note");
    note.textContent = notesList[i];
    ...
    deleteNote.addEventListener("click", () => {
      noteStorage.deleteNote(notesList[i]);
      loadNotes();
    });
...
function sortBookList(tabName) {
  if (tabName !== "hush" && tabName !== "numen") {
    ...
    const memory = memoryList.filter((mem) =>
      mem.aspect.includes(tabName.toLowerCase())
    );
    for (let i = 0; i < storage.getBooksMenu().books.length; i++) {
      ...
      if (memory.map((mem) => mem.memoryName).includes(book.memoryName)) {
        const bookMemory = memoryList.find(
          (mem) => mem.memoryName === book.memoryName
        );

        // ["lantern","foresight"] [1,2] aspect index => degree index

        const index = bookMemory.aspect.findIndex(
          (asp) => asp === tabName.toLowerCase()
        );
...
hushTab.addEventListener("click", () => {
  aspectTabs.forEach((tab) => tab.classList.remove("active"));
});
...
notesTab.addEventListener("click", () => {
  recipeTabs.forEach((tab) => tab.classList.remove("active"));
});
